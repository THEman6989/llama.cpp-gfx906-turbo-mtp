...
static void rope_multi_cuda(const T *            x,
    ...
    const gfx906_mrope_sections & gfx906_sections = reinterpret_cast<const gfx906_mrope_sections &>(sections);

    gfx906_rope_multi_cuda<forward, T>(
        x, dst, ne00, ne01, ne02, s1, s2, n_dims, nr,
        pos, freq_scale, freq_base, ext_factor, attn_factor,
        gfx906_corr, freq_factors, gfx906_sections, is_imrope, stream);
#else
    const dim3 block_dims(1, CUDA_ROPE_BLOCK_SIZE, 1);
    const int  n_blocks_x = (ne00 + 2 * CUDA_ROPE_BLOCK_SIZE - 1) / (2 * CUDA_ROPE_BLOCK_SIZE);
    const dim3 block_nums(nr, n_blocks_x, 1);
